...
class Layout extends Component {
  ...
  fetchGeneratedExam(status, page){
     ...
     apiRequest('get', '/user/no-of-years-as-als-teacher', false, this.props.token)
     	.then((res)=>{
     		console.log(res)
     		this.setState({
     			noOfYearsTeaching: res.data.noOfYearsAsAlsTeacher,
  ...
  render() {
  		...
  		let data =  {
			labels: labels,
			datasets: [{
			label: 'No of Years in ALS Teaching',
			data: ageData,
    ...
    return (
				    	...
				    	<Grid.Cell large={12} medium={12}  small={12}>
				    		<div className='bar-chart-label'>
								<div className='name'>No of Years in ALS Teaching</div>
